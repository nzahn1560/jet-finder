...
# Register Match Tool API
try:
    from match_tool_api import match_tool_bp
    app.register_blueprint(match_tool_bp)
    logger.info("✅ Match Tool API registered at /api/match-tool/*")
except ImportError as e:
    logger.warning(f"⚠️ Could not load Match Tool API: {e}")
...
try:
    ...
    logger.warning(f"⚠️ Could not load Admin API: {e}")
...
def load_aircraft_data():
    try:
        ...
        for _, row in df.iterrows():
            ...
            # Add random but reasonable aircraft and engine times for testing
            import random
            # Use aircraft ID as seed for consistency
            random.seed(aircraft['id'])
            
            # Determine if multi-engine based on aircraft type and name
            # Single-engine: typically turboprops, piston singles
            # Multi-engine: most jets, twin turboprops
            aircraft_type = str(row.get('Type', '')).lower()
            aircraft_name = str(row.get('316', '')).lower()
            manufacturer = str(row.get('Manufacturer', '')).lower()
            
            # Single-engine indicators (explicit single-engine aircraft)
            single_engine_keywords = ['cessna 172', 'cessna 182', 'cessna 206', 'piper cherokee', 'piper warrior', 'mooney', 'bonanza', 'cirrus']
            single_engine_patterns = ['single', 'se-', ' se ', 'single engine']
            
            # Multi-engine indicators (explicitly multi-engine)
            multi_engine_keywords = ['twin', 'citation', 'gulfstream', 'bombardier', 'falcon', 'hawker', 'challenger', 'global', 'phenom', 'embraer', 'king air', 'beechcraft', 'cessna 421', 'cessna 340', 'cessna 414']
            multi_engine_patterns = ['twin', 'multi', 'me-', ' me ']
            
            # Default: assume multi-engine for jets, single for turboprops
            is_multi_engine = True  # Default to multi-engine (most aircraft in dataset are jets)
            
            # Check aircraft type first
            if 'turboprop' in aircraft_type and 'twin' not in aircraft_type and 'twin' not in aircraft_name:
                # Turboprops are often single-engine unless explicitly "twin"
                is_multi_engine = False
            
            # Check for explicit single-engine indicators
            if any(keyword in aircraft_name or keyword in manufacturer for keyword in single_engine_keywords):
                is_multi_engine = False
            elif any(pattern in aircraft_name or pattern in aircraft_type for pattern in single_engine_patterns):
                is_multi_engine = False
            
            # Check for explicit multi-engine indicators (override single-engine check)
            if any(keyword in aircraft_name or keyword in manufacturer for keyword in multi_engine_keywords):
                is_multi_engine = True
            elif any(pattern in aircraft_name or pattern in aircraft_type for pattern in multi_engine_patterns):
                is_multi_engine = True
            
            # Jets are almost always multi-engine (except very rare single-engine jets)
            if 'jet' in aircraft_type or 'jet' in aircraft_name:
                # Only single-engine jets are very rare (like Cirrus Vision SF50)
                if 'vision' in aircraft_name and 'sf50' in aircraft_name:
                    is_multi_engine = False
                else:
                    is_multi_engine = True
            
            # Airframe time: 500-8000 hours (reasonable range)
            aircraft['total_time'] = random.randint(500, 8000)
            
            # Engine 1 time: 200-5000 hours
            aircraft['engine1_time'] = random.randint(200, 5000)
            
            # Engine 1 TBO: 3000-5000 hours (typical TBO range)
            aircraft['engine1_tbo'] = random.randint(3000, 5000)
            
            # Engine 2 (if multi-engine)
            if is_multi_engine:
                aircraft['engine2_time'] = random.randint(200, 5000)
                aircraft['engine2_tbo'] = random.randint(3000, 5000)
            else:
                aircraft['engine2_time'] = None
                aircraft['engine2_tbo'] = None
...
def home():
    ...
    return render_template('index.html', 
                         filtered_aircraft=filtered_aircraft,  # Pass ALL aircraft for client-side pagination
